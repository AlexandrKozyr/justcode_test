...
abstract class BaseSalePeer {
	...
	const CLASS_DEFAULT = 'lib.model.Sale';
	...
	{
		$criteria->addSelectColumn(SalePeer::ID);
		$criteria->addSelectColumn(SalePeer::TITLE);
		$criteria->addSelectColumn(SalePeer::CONTENT);
	}
	...
	public static function doCount(Criteria $criteria, $distinct = false, PropelPDO $con = null)
	{
		...
		{
		  call_user_func($sf_hook, 'BaseSalePeer', $criteria, $con);
		}
	...
	{
		return $withPrefix ? SalePeer::CLASS_DEFAULT : SalePeer::OM_CLASS;
	}

	/**
	 * Method perform an INSERT on the database, given a Sale or Criteria object.
	 *
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseSalePeer:doInsert:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseSalePeer', $values, $con))
      {
		...
		if ($criteria->containsKey(SalePeer::ID) && $criteria->keyContainsValue(SalePeer::ID) ) {
			throw new PropelException('Cannot insert a value for auto-increment primary key ('.SalePeer::ID.')');
		}
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseSalePeer:doInsert:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseSalePeer', $values, $con, $pk);
    }
	...
	/**
	 * Method perform an UPDATE on the database, given a Sale or Criteria object.
	 *
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseSalePeer:doUpdate:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseSalePeer', $values, $con))
      {
        return $sf_hook_retval;
		...
		if ($values instanceof Criteria) {
			...
			$comparison = $criteria->getComparison(SalePeer::ID);
			$selectCriteria->add(SalePeer::ID, $criteria->remove(SalePeer::ID), $comparison);
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseSalePeer:doUpdate:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseSalePeer', $values, $con, $ret);
    }
	...
	/**
	 * Method to DELETE all rows from the sale table.
	 *
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
	public static function doDeleteAll($con = null)
	{
		...
		try {
			...
			$con->beginTransaction();
			$affectedRows += BasePeer::doDeleteAll(SalePeer::TABLE_NAME, $con);
	...
	/**
	 * Method perform a DELETE on the database, given a Sale or Criteria object OR a primary key value.
	 *
	 ...
	 */
	 public static function doDelete($values, PropelPDO $con = null)
	 {
		...
		if ($values instanceof Criteria) {
			...
			$criteria->add(SalePeer::ID, (array) $values, Criteria::IN);
	 ...
	 */
	public static function doValidate(Sale $obj, $cols = null)
	{
		$columns = array();
	...
	public static function retrieveByPKs($pks, PropelPDO $con = null)
	{
		...
		if (empty($pks)) {
			...
			$criteria = new Criteria(SalePeer::DATABASE_NAME);
			$criteria->add(SalePeer::ID, $pks, Criteria::IN);
	...
	{
	  return array(array('id'), array('title'));
	}
	...
	static private function getMixerPreSelectHook($method)
	{
	  ...
	  {
	    return sprintf('BaseSalePeer:%s:%1$s', 'Count' == $match[1] ? 'doCount' : $match[0]);
	  }
